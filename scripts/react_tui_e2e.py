...
def _spawn_oh(*, env: dict[str, str] | None = None) -> pexpect.spawn:
    ...
    if os.environ.get("AGENTCHART_E2E_DEBUG") == "1":
...
def _isolated_env(permission_mode: str = "full_auto") -> tuple[tempfile.TemporaryDirectory[str], dict[str, str]]:
    ...
    temp_dir = tempfile.TemporaryDirectory(prefix="agentchart-react-tui-")
    ...
    env = os.environ.copy()
    env["AGENTCHART_CONFIG_DIR"] = str(config_dir)
    env["AGENTCHART_DATA_DIR"] = str(data_dir)
