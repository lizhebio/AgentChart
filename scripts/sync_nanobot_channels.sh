...
UPSTREAM_FILE="$REPO_ROOT/src/agentchart/channels/UPSTREAM"
CHANNELS_DEST="$REPO_ROOT/src/agentchart/channels"
...
for f in "$CHANNELS_DEST/bus/"*.py "$CHANNELS_DEST/impl/"*.py; do
  sed -i \
    -e 's/from nanobot\.bus\./from agentchart.channels.bus./g' \
    -e 's/from nanobot\.channels\./from agentchart.channels.impl./g' \
    -e 's/from nanobot\.config\.schema import/from agentchart.config.schema import/g' \
    -e 's/from nanobot\.utils\.helpers import/from agentchart.utils.helpers import/g' \
    -e 's/from nanobot\.config\.loader import/from agentchart.config.loader import/g' \
    "$f"
  # Replace loguru
  sed -i \
