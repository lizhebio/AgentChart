#!/usr/bin/env python3
"""Run real end-to-end AgentChart scenarios against an Anthropic-compatible API."""
...
from agentchart.engine import QueryEngine
...
from agentchart.memory import add_memory_entry
...
def _setup_plugin_combo_flow(cwd: Path, _: Path) -> None:
    plugin_dir = cwd / ".agentchart" / "plugins" / "fixture-plugin"
...
def _validate_worktree_flow(cwd: Path, final_text: str, tool_names: list[str], started: int, completed: int) -> tuple[bool, str]:
    worktree_path = cwd / ".agentchart" / "worktrees" / "smoke-worktree"
...
SCENARIOS: dict[str, Scenario] = {
    "file_io": Scenario(
        name="file_io",
        prompt=(
            "You are running an AgentChart smoke test. "
            "You must use tools. "
            "1. Use write_file to create smoke.txt with the exact content AGENTCHART_E2E_OK. "
    ...
    "search_edit": Scenario(
        name="search_edit",
        prompt=(
            "You are running an AgentChart search and edit test. "
            "You must use tools. "
    ...
    "phase48": Scenario(
        name="phase48",
        prompt=(
            "You are running an AgentChart Phase 4-8 smoke test. "
            "You must use tools. "
    ...
    "task_flow": Scenario(
        name="task_flow",
        prompt=(
            "You are running an AgentChart background task test. "
            "You must use tools. "
    ...
    "skill_flow": Scenario(
        name="skill_flow",
        prompt=(
            "You are running an AgentChart skill loading test. "
            "You must use tools. "
    ...
    "mcp_model": Scenario(
        name="mcp_model",
        prompt=(
            "You are running an AgentChart MCP integration test. "
            "You must use tools. "
    ...
    "mcp_resource": Scenario(
        name="mcp_resource",
        prompt=(
            "You are running an AgentChart MCP resource test. "
            "You must use tools. "
    ...
    "agent_flow": Scenario(
        name="agent_flow",
        prompt=(
            "You are running an AgentChart agent delegation test. "
            "You must use tools. "
    ...
    "remote_agent_flow": Scenario(
        ...
        prompt=(
            "You are running an AgentChart remote-agent mode test. "
            "You must use tools. "
    ...
    "plugin_combo": Scenario(
        name="plugin_combo",
        prompt=(
            "You are running an AgentChart plugin combo test. "
            "You must use tools. "
    ...
    "ask_user_flow": Scenario(
        name="ask_user_flow",
        prompt=(
            "You are running an AgentChart ask-user test. "
            "You must use tools. "
    ...
    "task_update_flow": Scenario(
        ...
        prompt=(
            "You are running an AgentChart task update test. "
            "You must use tools. "
    ...
    "notebook_flow": Scenario(
        name="notebook_flow",
        prompt=(
            "You are running an AgentChart notebook test. "
            "You must use tools. "
    ...
    "lsp_flow": Scenario(
        name="lsp_flow",
        prompt=(
            "You are running an AgentChart LSP test. "
            "You must use tools. "
    ...
    "cron_flow": Scenario(
        name="cron_flow",
        prompt=(
            "You are running an AgentChart cron test. "
            "You must use tools. "
    ...
    "worktree_flow": Scenario(
        name="worktree_flow",
        prompt=(
            "You are running an AgentChart worktree test. "
            "You must use tools. "
    ...
    "mcp_auth_flow": Scenario(
        name="mcp_auth_flow",
        prompt=(
            "You are running an AgentChart MCP auth reconfiguration test. "
            "You must use tools. "
...
async def _run() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="agentchart-e2e-suite-") as temp_dir:
        ...
        os.environ["AGENTCHART_CONFIG_DIR"] = str(suite_root / "config")
        os.environ["AGENTCHART_DATA_DIR"] = str(suite_root / "data")
        try:
