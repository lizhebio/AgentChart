...
PLUGINS_REPO = Path("/tmp/agentchart-test-plugins/plugins")
...
async def test_install_real_skills() -> tuple[bool, str]:
    """Copy real skills from anthropics/skills into agentchart user skills dir."""
    from agentchart.config.paths import get_config_dir
...
async def test_real_skills_loaded() -> tuple[bool, str]:
    ...
    from agentchart.skills.loader import load_skill_registry
...
async def test_skills_in_prompt_with_real() -> tuple[bool, str]:
    ...
    from agentchart.prompts.context import build_runtime_system_prompt
...
async def test_install_real_plugins() -> tuple[bool, str]:
    """Copy real plugins into agentchart plugin directory."""
    if not PLUGINS_REPO.exists():
        return False, f"Plugins repo not found at {PLUGINS_REPO}"

    dest_base = PROJECT_ROOT / ".agentchart" / "plugins"
...
async def test_real_plugins_loaded() -> tuple[bool, str]:
    ...
    from agentchart.config.settings import load_settings
    from agentchart.plugins.loader import load_plugins
...
async def test_plugin_hook_structure() -> tuple[bool, str]:
    ...
    dest = PROJECT_ROOT / ".agentchart" / "plugins" / "security-guidance"
...
async def test_commit_command_content() -> tuple[bool, str]:
    ...
    dest = PROJECT_ROOT / ".agentchart" / "plugins" / "commit-commands"
