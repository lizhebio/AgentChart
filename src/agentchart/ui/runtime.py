...
from agentchart.api.client import AnthropicApiClient, SupportsStreamingMessages
...
from agentchart.commands import CommandContext, CommandResult, create_default_command_registry
from agentchart.config import get_config_file_path, load_settings
from agentchart.config.settings import display_model_setting
from agentchart.engine import QueryEngine
from agentchart.engine.messages import ConversationMessage, ToolResultBlock, ToolUseBlock
from agentchart.engine.query import MaxTurnsExceeded
from agentchart.engine.stream_events import StreamEvent
from agentchart.hooks import HookEvent, HookExecutionContext, HookExecutor, load_hook_registry
from agentchart.hooks.hot_reload import HookReloader
...
from agentchart.tools import ToolRegistry, create_default_tool_registry
from agentchart.keybindings import load_keybindings
...
class RuntimeBundle:
    ...
    def current_settings(self):
        ...
        We persist most settings to disk (``~/.agentchart/settings.json``), but
...
def _resolve_api_client_from_settings(settings) -> SupportsStreamingMessages:
    ...
    if settings.api_format == "copilot":
        from agentchart.api.copilot_client import COPILOT_DEFAULT_MODEL
...
async def build_runtime(
    *,
    ...
    """Build the shared runtime for an AgentChart session."""
