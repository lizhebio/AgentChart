...
TEAMMATE_COMMAND_ENV_VAR = "AGENTCHART_TEAMMATE_COMMAND"
...
_TEAMMATE_ENV_VARS = [
    ...
    "REQUESTS_CA_BUNDLE",
    "CURL_CA_BUNDLE",
    # --- AgentChart-native provider settings --------------------------------
...
def get_teammate_command() -> str:
    ...
    Resolution order:
    1. ``AGENTCHART_TEAMMATE_COMMAND`` environment variable — allows the
       operator to point at a specific binary or wrapper script.
    2. The ``agentchart`` entry-point on PATH (installed package mode).
    3. The current Python interpreter running the ``agentchart`` module
    ...
    entry_point = shutil.which("agentchart")
    if entry_point:
        return entry_point
...
def build_inherited_env_vars() -> dict[str, str]:
    ...
    Always includes ``AGENTCHART_AGENT_TEAMS=1`` plus any provider/proxy
