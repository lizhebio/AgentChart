"""Persistent team lifecycle management for AgentChart swarms.

Teams are stored as JSON files on disk:
    ~/.agentchart/teams/<name>/team.json
...
from agentchart.swarm.mailbox import get_team_dir
from agentchart.swarm.types import BackendType
...
async def _kill_orphaned_teammate_panes(team_name: str) -> None:
    ...
    """
    from agentchart.swarm.registry import get_backend_registry
    from agentchart.swarm.spawn_utils import is_inside_tmux
    from agentchart.swarm.types import is_pane_backend
...
class TeamLifecycleManager:
    ...
    Persists team metadata to ``~/.agentchart/teams/<name>/team.json``.
    ...
    def list_teams(self) -> list[TeamFile]:
        """Return all teams found in ``~/.agentchart/teams/``, sorted by name."""
        base = Path.home() / ".agentchart" / "teams"
        if not base.exists():
            return []
