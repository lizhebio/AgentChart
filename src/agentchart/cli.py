...
def _version_callback(value: bool) -> None:
    if value:
        print(f"agentchart {__version__}")
        raise typer.Exit()
...
app = typer.Typer(
    name="agentchart",
    help=(
        "AgentChart: declarative agent packaging and a local harness runtime.\n\n"
...
def mcp_add(
    ...
    from agentchart.config import load_settings, save_settings
...
def plugin_install(
    ...
    from agentchart.plugins.installer import install_plugin_from_path
...
def plugin_uninstall(
    ...
    """Uninstall a plugin."""
    from agentchart.plugins.installer import uninstall_plugin
...
def cron_start() -> None:
    ...
    from agentchart.services.cron_scheduler import is_scheduler_running, start_daemon
...
def cron_stop() -> None:
    ...
    from agentchart.services.cron_scheduler import stop_scheduler
...
def cron_status_cmd() -> None:
    ...
    from agentchart.services.cron_scheduler import scheduler_status
...
def cron_list_cmd() -> None:
    ...
    from agentchart.services.cron import load_cron_jobs
...
def cron_toggle_cmd(
    ...
    from agentchart.services.cron import set_job_enabled
...
def cron_history_cmd(
    ...
    from agentchart.services.cron_scheduler import load_history
...
def cron_logs_cmd(
    ...
    from agentchart.config.paths import get_logs_dir

    log_path = get_logs_dir() / "cron_scheduler.log"
    if not log_path.exists():
        print("No scheduler log found. Start the scheduler with: ac cron start")
        return
...
def _bind_external_provider(provider: str) -> None:
    ...
    from agentchart.auth.external import default_binding_for_provider, load_external_credential
    from agentchart.auth.storage import store_external_binding
    ...
    print(f"Use `ac provider use {profile_name}` to activate it.", flush=True)
...
def auth_login(
    ...
    """
    from agentchart.auth.flows import ApiKeyFlow
    from agentchart.auth.manager import AuthManager
    from agentchart.auth.storage import store_credential
...
def _run_copilot_login() -> None:
    ...
    from agentchart.api.copilot_auth import save_copilot_auth
    from agentchart.auth.flows import DeviceCodeFlow
    ...
    print("  ac provider use copilot", flush=True)


@auth_app.command("copilot-login")
def auth_copilot_login() -> None:
    """Authenticate with GitHub Copilot via device flow (alias for 'ac auth login copilot')."""
    _run_copilot_login()


@auth_app.command("codex-login")
def auth_codex_login() -> None:
    """Bind AgentChart to a local Codex CLI subscription session."""
    _bind_external_provider("openai_codex")


@auth_app.command("claude-login")
def auth_claude_login() -> None:
    """Bind AgentChart to a local Claude CLI subscription session."""
...
def auth_copilot_logout() -> None:
    ...
    from agentchart.api.copilot_auth import clear_github_token
...
def provider_add(
    ...
    from agentchart.auth.manager import AuthManager
    from agentchart.config.settings import ProviderProfile
...
def main(
    ...
    if theme:
        from agentchart.config.settings import load_settings, save_settings

        settings = load_settings()
        settings.theme = theme
        save_settings(settings)

    from agentchart.ui.app import run_print_mode, run_repl
