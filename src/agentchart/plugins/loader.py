...
from agentchart.plugins.schemas import PluginManifest
from agentchart.plugins.types import LoadedPlugin
from agentchart.skills.loader import _parse_skill_markdown
...
def get_project_plugins_dir(cwd: str | Path) -> Path:
    ...
    path = Path(cwd).resolve() / ".agentchart" / "plugins"
    path.mkdir(parents=True, exist_ok=True)
    return path
...
def _load_plugin_mcp(path: Path) -> dict[str, object]:
    ...
    if not path.exists():
        return {}
    from agentchart.mcp.types import McpJsonConfig
