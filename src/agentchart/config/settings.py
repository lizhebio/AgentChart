"""Settings model and loading logic for AgentChart.

Settings are resolved with the following precedence (highest first):
1. CLI arguments
2. Environment variables (ANTHROPIC_API_KEY, AGENTCHART_MODEL, etc.)
3. Config file (~/.agentchart/settings.json)
4. Defaults
"""
...
from agentchart.hooks.schemas import HookDefinition
from agentchart.mcp.types import McpServerConfig
...
class Settings(BaseModel):
    """Main settings model for AgentChart."""
    ...
    def resolve_api_key(self) -> str:
        ...
        ``ac auth copilot-login`` and this method is not called.
    ...
    def resolve_auth(self) -> ResolvedAuth:
        ...
        if auth_source in {"codex_subscription", "claude_subscription"}:
            ...
            from agentchart.auth.storage import load_external_binding
            ...
            if binding is None:
                raise ValueError(
                    f"No external auth binding found for {auth_source}. Run 'ac auth "
        ...
        from agentchart.auth.storage import load_credential
...
def load_settings(config_path: Path | None = None) -> Settings:
    ...
    if config_path is None:
        from agentchart.config.paths import get_config_file_path
