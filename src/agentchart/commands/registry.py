...
from agentchart.bridge import get_bridge_manager
from agentchart.bridge.types import WorkSecret
from agentchart.bridge.work_secret import build_sdk_url, decode_work_secret, encode_work_secret
from agentchart.api.provider import auth_status, detect_provider
from agentchart.config.settings import Settings, display_model_setting, load_settings, save_settings
from agentchart.engine.messages import ConversationMessage
from agentchart.engine.query_engine import QueryEngine
from agentchart.memory import (
    add_memory_entry,
...
from agentchart.output_styles import load_output_styles
from agentchart.permissions import PermissionChecker, PermissionMode
from agentchart.plugins import load_plugins
from agentchart.prompts import build_runtime_system_prompt
from agentchart.plugins.installer import install_plugin_from_path, uninstall_plugin
from agentchart.services import compact_messages, estimate_conversation_tokens, summarize_messages
from agentchart.services.session_backend import DEFAULT_SESSION_BACKEND, SessionBackend
from agentchart.skills import load_skill_registry
from agentchart.tasks import get_task_manager

if TYPE_CHECKING:
    from agentchart.state import AppStateStore
...
def create_default_command_registry() -> CommandRegistry:
    ...
    async def _version_handler(_: str, context: CommandContext) -> CommandResult:
        del context
        try:
            version = importlib.metadata.version("agentchart")
        except importlib.metadata.PackageNotFoundError:
            version = "0.1.0"
        return CommandResult(message=f"AgentChart {version}")
    ...
    async def _init_handler(args: str, context: CommandContext) -> CommandResult:
        ...
        if not claudemd.exists():
            claudemd.write_text(
                ...
                "- Use AgentChart tools deliberately.\n"
        ...
        for relative, content in (
            (
                ...
                "# Project AgentChart Config\n\nThis directory stores project-specific AgentChart state.\n",
            ),
            (
        ...
        if not created:
            return CommandResult(message="Project already initialized for AgentChart.")
    ...
    async def _login_handler(args: str, context: CommandContext) -> CommandResult:
        ...
        return CommandResult(message="Stored API key in ~/.agentchart/settings.json")
    ...
    async def _theme_handler(args: str, context: CommandContext) -> CommandResult:
        from agentchart.themes import list_themes, load_theme
    ...
    async def _keybindings_handler(_: str, context: CommandContext) -> CommandResult:
        from agentchart.keybindings import get_keybindings_path, load_keybindings
    ...
    async def _voice_handler(args: str, context: CommandContext) -> CommandResult:
        from agentchart.voice import extract_keyterms, inspect_voice_capabilities
    ...
    async def _privacy_settings_handler(_: str, context: CommandContext) -> CommandResult:
        ...
        lines = [
            ...
            "- storage: local files under ~/.agentchart and project .agentchart",
        ]
        return CommandResult(message="\n".join(lines))
    ...
    async def _release_notes_handler(_: str, context: CommandContext) -> CommandResult:
        ...
        return CommandResult(
            message=(
                "# Release Notes\n\n"
                "- React TUI is now the default `ac` interface.\n"
    ...
    registry.register(SlashCommand("exit", "Exit AgentChart", _exit_handler))
    registry.register(SlashCommand("clear", "Clear conversation history", _clear_handler))
    registry.register(SlashCommand("version", "Show the installed AgentChart version", _version_handler))
    ...
    registry.register(SlashCommand("init", "Initialize project AgentChart files", _init_handler))
    ...
    registry.register(SlashCommand("release-notes", "Show recent AgentChart release notes", _release_notes_handler))
