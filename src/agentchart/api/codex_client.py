...
from agentchart.api.client import (
...
from agentchart.api.errors import AuthenticationFailure, AgentChartApiError, RateLimitFailure, RequestFailure
...
def _build_codex_headers(token: str, *, session_id: str | None = None) -> dict[str, str]:
    ...
    headers = {
        ...
        "originator": "agentchart",
        "User-Agent": f"agentchart ({platform.system().lower()} {platform.machine() or 'unknown'})",
...
def _translate_status_error(status_code: int, message: str) -> AgentChartApiError:
...
class CodexApiClient:
    ...
    async def _stream_once(self, request: ApiMessageRequest) -> AsyncIterator[ApiStreamEvent]:
        body: dict[str, Any] = {
            ...
            "store": False,
            "stream": True,
            "instructions": request.system_prompt or "You are AgentChart.",
    ...
    @staticmethod
    def _translate_error(exc: Exception) -> AgentChartApiError:
        if isinstance(exc, AgentChartApiError):
            return exc
        if isinstance(exc, httpx.HTTPStatusError):
