"""GitHub Copilot API client for AgentChart.
...
from agentchart.api.errors import AuthenticationFailure
...
_VERSION = "0.1.0"  # AgentChart version for User-Agent
...
class CopilotClient:
    ...
    github_token:
        ...
        token is loaded from ``~/.agentchart/copilot_auth.json``.
    enterprise_url:
        ...
        *,
        ...
        if not token:
            raise AuthenticationFailure(
                "No GitHub Copilot token found. Run 'ac auth copilot-login' first."
            )
        ...
        default_headers: dict[str, str] = {
            "User-Agent": f"agentchart/{_VERSION}",
