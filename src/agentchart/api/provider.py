...
from agentchart.auth.external import describe_external_binding
from agentchart.auth.storage import load_external_binding
from agentchart.api.registry import detect_provider_from_registry
...
def auth_status(settings: Settings) -> str:
    ...
    if settings.api_format == "copilot":
        from agentchart.api.copilot_auth import load_copilot_auth

        auth_info = load_copilot_auth()
        if not auth_info:
            return "missing (run 'ac auth copilot-login')"
    ...
    try:
        ...
        if settings.provider == "openai_codex":
            return "missing (run 'ac auth codex-login')"
        if settings.provider == "anthropic_claude":
            ...
            return "missing (run 'ac auth claude-login')"
        return "missing"
