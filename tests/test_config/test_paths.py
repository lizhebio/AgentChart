"""Tests for agentchart.config.paths."""
...
def test_get_config_file_path(tmp_path: Path, monkeypatch):
    ...
    assert path == tmp_path / ".agentchart" / "settings.json"


def test_get_data_dir_default(tmp_path: Path, monkeypatch):
    monkeypatch.delenv("AGENTCHART_CONFIG_DIR", raising=False)
    monkeypatch.delenv("AGENTCHART_DATA_DIR", raising=False)
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: tmp_path))
    data_dir = get_data_dir()
    assert data_dir == tmp_path / ".agentchart" / "data"
    assert data_dir.is_dir()


def test_get_data_dir_env_override(tmp_path: Path, monkeypatch):
    custom = tmp_path / "custom_data"
    monkeypatch.setenv("AGENTCHART_DATA_DIR", str(custom))
    data_dir = get_data_dir()
...
def test_get_logs_dir_default(tmp_path: Path, monkeypatch):
    monkeypatch.delenv("AGENTCHART_CONFIG_DIR", raising=False)
    monkeypatch.delenv("AGENTCHART_LOGS_DIR", raising=False)
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: tmp_path))
    logs_dir = get_logs_dir()
    assert logs_dir == tmp_path / ".agentchart" / "logs"
    assert logs_dir.is_dir()


def test_get_logs_dir_env_override(tmp_path: Path, monkeypatch):
    custom = tmp_path / "custom_logs"
    monkeypatch.setenv("AGENTCHART_LOGS_DIR", str(custom))
    logs_dir = get_logs_dir()
