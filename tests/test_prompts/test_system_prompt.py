"""Tests for agentchart.prompts.system_prompt."""

from __future__ import annotations

from agentchart.prompts.environment import EnvironmentInfo
...
def test_build_system_prompt_custom_prompt():
    ...
    assert "AgentChart" not in prompt


def test_build_system_prompt_default_includes_base():
    env = _make_env()
    prompt = build_system_prompt(env=env)
    assert "AgentChart" in prompt
