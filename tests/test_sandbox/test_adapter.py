...
from agentchart.config.settings import SandboxSettings, Settings
from agentchart.sandbox.adapter import (
    SandboxUnavailableError,
    build_sandbox_runtime_config,
    get_sandbox_availability,
    wrap_command_for_sandbox,
)
from agentchart.utils.shell import create_shell_subprocess
...
def test_sandbox_availability_reports_native_windows_unsupported(monkeypatch):
    ...
    monkeypatch.setattr("agentchart.sandbox.adapter.get_platform", lambda: "windows")
...
def test_wrap_command_for_sandbox_writes_settings_file(monkeypatch):
    ...
    monkeypatch.setattr("agentchart.sandbox.adapter.get_platform", lambda: "linux")
    monkeypatch.setattr("agentchart.sandbox.adapter.shutil.which", fake_which)
...
def test_wrap_command_for_sandbox_raises_when_required(monkeypatch):
    ...
    monkeypatch.setattr("agentchart.sandbox.adapter.get_platform", lambda: "linux")
    monkeypatch.setattr("agentchart.sandbox.adapter.shutil.which", lambda name: None)
...
def test_create_shell_subprocess_preserves_exit_code_with_sandbox(monkeypatch):
    import agentchart.config.paths as config_paths

    async def _run() -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            cfg = Path(tmpdir) / "settings.json"
            from agentchart.config.settings import save_settings
            ...
            try:
                process = await create_shell_subprocess(
                    "exit 7",
                    cwd=Path("/home/tangjiabin/AgentChart-new"),
