...
from agentchart.api.client import AnthropicApiClient, OAUTH_BETA_HEADER
...
def test_anthropic_client_adds_oauth_beta_header(monkeypatch):
    ...
    monkeypatch.setattr("agentchart.api.client.AsyncAnthropic", _FakeAsyncAnthropic)
...
def test_anthropic_client_adds_claude_oauth_identity_headers(monkeypatch):
    ...
    monkeypatch.setattr("agentchart.api.client.AsyncAnthropic", _FakeAsyncAnthropic)
    monkeypatch.setattr(
        "agentchart.auth.external.get_claude_code_version",
        lambda: "2.1.92",
    )
    monkeypatch.setattr(
        "agentchart.auth.external.get_claude_code_session_id",
        lambda: "session-123",
    )
    monkeypatch.setattr(
        "agentchart.api.client.get_claude_code_session_id",
        lambda: "session-123",
    )
    monkeypatch.setattr(
        "agentchart.api.client.claude_attribution_header",
...
def test_anthropic_client_refreshes_claude_token_on_request(monkeypatch):
    ...
    from agentchart.api.client import ApiMessageRequest
    ...
    assert client._client.beta.messages.last_params["metadata"] == {
        "user_id": '{"device_id":"agentchart","session_id":"session-123","account_uuid":""}'
    }
